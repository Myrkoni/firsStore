...
export default function Header() {
  ...
  // const [phones] = useGetAllProducts("phones");
  const [phones] = useAllProducts("phones");
  const [accessories] = useAllProducts("accessories")
  const [watches] = useAllProducts("watches");
  // const [accessories] = useGetAllProducts("accessories");
  // const [watches] = useGetAllProducts("watches");
    ...
    <header className={styles.header}>
      ...
      <nav className={styles.nav}>
        ...
        <div className={styles.dropdown}>
          <Link to='/products/phones' className={styles.dropBtn} >Телефони</Link>
          <div className={styles.menu}>
            {phones.length > 0
              ? phones.map(phone => <Link to={`/products/phones/brand/${phone.brand}`} key={phone._id}>{phone.name}</Link>)
        ...
        <div className={styles.dropdown}>
          <Link to='/products/watches' className={styles.dropBtn} >Часовници</Link>
          {/* <div className={styles.menu}>
            {watches.length > 0
        ...
        <div className={styles.dropdown}>
          <Link to='/products/accessories' className={styles.dropBtn} >Аксесоари</Link>
          <div className={styles.menu}>
            {accessories.length > 0
              ? accessories.map(accessor => <Link to={`/products/accessories/brand/${accessor.brand}`} key={accessor._id}>{accessor.name}</Link>)
              : <p style={{ color: "white" }}>Няма налични продукти</p>
            }
          </div>
        </div>

        {fullName === "Admin" && <Link to="/products/create">Добави продукт</Link>}
      ...
      <div className={styles.right}>
        ...
        <div className={styles.cart}>
          <button className={styles.iconBtn}><Link to='/cart'>🛒</Link></button>
        ...
        {!isAuthenticated ? (
          ...
          <>
            <Link to='/profile' className={styles.profileBtn}>Профил</Link>
            <Link to='/logout' className={styles.logoutBtn}>Изход</Link>
          </>
        )}
      </div>
    </header>
